...
// ── getUserMedia ──────────────────────────────────────────────────────────────
// Phase 4.22 — dead declaration removed. The Kotlin-side `getUserMedia()`
// external declared echo/AGC/NS constraints but no file calls it; we rely on
// `window.__tbStream` set up by `index.html` BEFORE Kotlin loads (that's the
// pre-acquire strategy that lets us skip an async getUserMedia inside Kotlin
// coroutines, which froze Safari). Audio constraints now lifted into
// `index.html.acquireStream()` so they actually take effect.
...
// ── Global stream cache ───────────────────────────────────────────────────────
// Phase 1.16 — was process-global + never disposed between calls. Still
// process-global (we cache the pre-acquired browser stream so we don't
// pay a fresh getUserMedia() on every init), but `dispose()` now stops
// all tracks in this stream AND nulls it so subsequent code can rebind.
