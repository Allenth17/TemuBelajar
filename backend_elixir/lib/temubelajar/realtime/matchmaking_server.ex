defmodule TemuBelajar.Realtime.MatchmakingServer do
  @moduledoc """
  ...
  def handle_call({:join_queue, email, university}, _from, state) do
    ...
    if already_queued do
      ...
      case find_best_match(state.queue, email, university) do
        ...
        {peer_email, peer_university, _ts} ->
          ...
          {:reply, {:matched, pair_id, peer_email, peer_university, university},
           %{state | queue: new_queue, pairs: new_pairs}}
      end
    end
  end
  ...
  def handle_info(:heartbeat, state) do
    ...
    {alive, expired} =
      Enum.split_with(state.queue, fn {_, _, ts} ->
